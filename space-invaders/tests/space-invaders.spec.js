...
test.beforeEach(async ({ page }) => {
  await page.addInitScript(() => { try { localStorage.setItem('space-invaders-help-seen', '1'); } catch {} });
  const runtimeErrors = [];
...
test.describe('how to play help', () => {
  async function clearHelpSeenOnce(page) {
    await page.addInitScript(() => {
      try {
        if (!localStorage.getItem('space-invaders-help-clear-done')) {
          localStorage.removeItem('space-invaders-help-seen');
          localStorage.setItem('space-invaders-help-clear-done', '1');
        }
      } catch {}
    });
  }
  ...
  test('dismissing help sets the seen flag, unpauses, and stays hidden after reload', async ({ page }) => {
    ...
    expect(state.paused).toBe(false);
    const flag = await page.evaluate(() => localStorage.getItem('space-invaders-help-seen'));
    expect(flag).toBe('1');
