...
test.beforeEach(async ({ page }) => {
  await page.addInitScript(() => { try { localStorage.setItem('asteroids-help-seen', '1'); } catch {} });
  const runtimeErrors = [];
  page.on('console', (msg) => {
...
test.describe('how to play help', () => {
  async function clearHelpSeenOnce(page) {
    await page.addInitScript(() => {
      try {
        if (!localStorage.getItem('asteroids-help-clear-done')) {
          localStorage.removeItem('asteroids-help-seen');
          localStorage.setItem('asteroids-help-clear-done', '1');
        }
      } catch {}
    });
  }
  ...
  test('dismissing help sets the seen flag, unpauses, and stays hidden after reload', async ({ page }) => {
    ...
    expect(state.paused).toBe(false);
    const flag = await page.evaluate(() => localStorage.getItem('asteroids-help-seen'));
    expect(flag).toBe('1');
