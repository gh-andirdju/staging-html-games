...
Layout is an arcade-cabinet: a `.cabinet` chassis (bezel + corner bolts) wraps the `.game-shell` grid (marquee / play area / control deck). The play area flanks the board with two fixed-width `.rail`s — HOLD + scoreboard on the left, a 3-deep NEXT queue (`#next-canvas` plus `.next-mini` `#next-canvas-2/3`, fed by `state.nextQueue`) and the status ticker on the right. `computeDimensions()` sizes cells by the tighter of available height *and* width (measured from the shell rect minus the rails) so the board never overflows; `.game-shell` pins `grid-template-columns: minmax(0, 1fr)` so rows can't expand to content width. The control deck keeps the two-cluster geometry (rotate cluster left, D-pad right) the ergonomics tests assert.
